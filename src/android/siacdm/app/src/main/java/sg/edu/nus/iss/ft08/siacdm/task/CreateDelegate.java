...
public class CreateDelegate extends AsyncTask<Task, Void, Task> {
  private TaskDetailController controller;
  private RestClient client;

  CreateDelegate(TaskDetailController controller) {
  ...
  protected Task doInBackground(Task... params) {
    ...
    Task result = null;
    try {
      String data = client.put(Urls.forTask(), task.toJson());
